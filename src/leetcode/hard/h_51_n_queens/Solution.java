package leetcode.hard.h_51_n_queens;
...
class Solution {
    ...
    private static void depthFirstSearch(String[][] board, int n, int[] columns, int[] diagonals, int[] antiDiagonals, int row, List<List<String>> resultPossibleSolutionsList) {
        ...
        for (int col = 0; col < n; ++col) {
            if (columns[col] + antiDiagonals[row + col] + diagonals[n - (row - col)] == 0) {
                board[row][col] = "Q";
                columns[col] = antiDiagonals[row + col] = diagonals[n - (row - col)] = 1;
                depthFirstSearch(board, n, columns, diagonals, antiDiagonals, row + 1, resultPossibleSolutionsList);
                columns[col] = antiDiagonals[row + col] = diagonals[n - (row - col)] = 0;
                board[row][col] = ".";
            }
        }
    }
